...
const opts = {
  strings: ['FELIZ', '8', 'MESES','AMOR'], // Cambiado a "FELIZ CUMPLEAÑOS AMOR"
  charSize: 50, // Aumentado el tamaño de las letras
  charSpacing: 50, // Aumentado el espaciado entre letras
  lineHeight: 60, // Aumentado el espaciado entre líneas
  cx: w / 2,
  cy: h / 2,
...
const calc = {
  totalWidth: opts.charSpacing * Math.max(opts.strings[0].length, opts.strings[1].length, opts.strings[2].length), // Ajustado para 3 líneas
};
...
ctx.font = `${opts.charSize}px Verdana`; // Tamaño de fuente más grande
  ...
  step() {
    if (this.phase === 'firework') {
      ...
      if (this.tick > opts.letterContemplatingWaitTime) {
        ...
        this.vx = Math.cos(rad) * (opts.balloonBaseVel + opts.balloonAddedVel * Math.random());
        this.vy = Math.sin(rad) * (opts.balloonBaseVel + opts.balloonAddedVel * Math.random());
      }
      ...
      if (this.spawning) {
        ...
        if (this.cy + this.size < -hh || this.cx < -hw || this.cy > hh) this.phase = 'done';
      }
    }
  }
}
...
document.getElementById('playButton').addEventListener('click', function() {
  // Abrir una nueva pestaña con una página que reproduce la música
  const newWindow = window.open('music.html', '_blank');
  
  // Enfocar la nueva pestaña (opcional)
  if (newWindow) {
    newWindow.focus();
  }
});